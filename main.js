...
buttonAdd.addEventListener('click', () => {
    ...
    function doneTask(id) {
        let newDoneTask = tasks.filter(i => i.id === parseFloat(id))
...
function removeTask(id) {
    ...
    let newTask = tasks.filter(i => i.id !== parseFloat(id))

    tasks = newTask;    
    
    console.log(newTask, id)

    localStorage.setItem('dataTasks', JSON.stringify(newTask))
    handleFetch()
}
...
function checkLocalStorage() {
    const lc = localStorage.getItem('dataTasks');
    if(lc) {
         tasks = JSON.parse(lc);
    } else {
        localStorage.setItem('dataTasks', JSON.stringify(tasks))
...
handleFetch()
>>>>>>> 7d393608bfccd27923ad6bb8e65322bd14fe9110
